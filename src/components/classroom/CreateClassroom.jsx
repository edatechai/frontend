...
const Index = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (
      window.confirm(
        "Are you sure you want to delete this class room?, this action can not be undone, also not that all the paticipants in this class will be deleted too"
      )
    ) {
      try {
          ...
          <div className="mt-4">
            <label className="form-control w-full min-w-full">
              ...
              <input
                ...
                type="text"
                placeholder="Class name e.g Jss 1 A"
              ...
              {classRooms?.map((i, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td>{i.numberOfStudents.length}</td>
                    <td>{i.numberOfTeachers.length}</td>
                    <th>
