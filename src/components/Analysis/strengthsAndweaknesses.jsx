...
const StrengthsAndweaknesses = (props) => {
  ...
  const [getSW, {data, isLoading, isError, isSuccess}] = useGetStrengthsAndweaknessesMutation()
  const [realData, setRealData] = useState()
  
  useEffect(() => {
    function getCategoryAndTopic(objCode, props) {
      const result = props?.quizResult?.data?.find((result) => result.objCode === objCode);
      if (result) {
  ...
  const chartData = realData?.data?.aggData?.chartData
  ...
  const newdata = [
    {
      ...
      student: chartData?.studentScores.studentScore,
      class: chartData?.classScores.classScore,
      country: chartData?.countryScores.countryScore, 
    },
    // {
    //   name: 'South',
    //   sales1: 59,
...
<dialog id="my_modal_5" className="modal modal-bottom sm:modal-middle">
  <div className="modal-box">
    <h3 className="font-bold text-lg">Let's Analyse your strengths and weeknesses</h3>
    <div>
    <select 
//   disabled={isLoading} 
  onChange={(e) => setClassId(e.target.value)} className="select select-bordered w-full mt-3">
    <option disabled selected>Select your classroom</option>
  ...
  onChange={(e) => setObjCode(e.target.value)} className="select select-bordered w-full mt-3">
    ...
    <div className="modal-action">
      <form method="dialog">
        ...
        <button onClick={handleSw} className="btn">Submit</button>
      </form>
    </div>
  </div>
</dialog>
     ...
     <div className='font-medium text-[25px] mb-5'>Mathematics Jss 1 A</div>
      <button onClick={()=>document.getElementById('my_modal_5').showModal()} className='btn bg-blue-500 text-white mb-5'>Analyse your strengths and weeknesses</button>
     </div>
      <div>
        <div>
        <div className="collapse collapse-plus bg-white">
  <input type="radio" name="my-accordion-3" defaultChecked />
  <div className="collapse-title  font-medium text-green-600">Strengths</div>
  <div className="collapse-content">
  <ul tabIndex={0} className="  z-[1] w-full">
    {
      realData?.data?.strengths.map((i, index)=>{
        return(
          <li key={index} className="flex justify-between flex-row">
...
<div className="collapse collapse-plus bg-white mt-5">
  <input type="radio" name="my-accordion-3" defaultChecked />
  <div className="collapse-title  font-medium text-red-500">Weaknesses</div>
  <div className="collapse-content">
  <ul tabIndex={0} className="  z-[1] w-full">
    {
      realData?.data?.weaknesses.map((i, index)=>{
        return(
          <li key={index} className="flex justify-between flex-row">
            <a> Topic: {i?.objective_name}</a>
            <a> Score: {i?.score}</a>
