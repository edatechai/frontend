...
import { ClassResult, QuizResult, useResultsByClassIdQuery } from "@/features/api/apiSlice";
...
// Removed local QuizResult type as it is imported from apiSlice

function findValue(array: ClassResult[], obj: string, name: string) {
...
function findResults(array: ClassResult[], obj: string, name: string) {
  for (let i = 0; i < array.length; i++) {
    if (array[i]?.objective === obj && array[i].userInfo.fullName === name) {
