...
function ChatBot({ userInfo, rec, onClose }: ChatBotProps) {
  ...
  useEffect(() => {
    if (conversation.length > 0) {
      const items = conversation.reduce((acc: ConversationItem[], message: Message) => {
        if (message.role === 'user') {
          ...
          if (acc.length > 0 && acc[acc.length - 1].type === 'assistant') {
            ...
            acc.push({ type: 'assistant', messages: [message], currentIndex: 0 });
          }
        }
        return acc;
      }, []);
  ...
  const handleQuestionSubmit = async () => {
    if (question.trim() && !isSubmitting) {
      ...
      try {
        ...
        setConversation(response?.data.conversation)
        setAskedQuestions(response?.data.askedQuestions)
        setConversationItems(prev => [...prev, { type: 'user', messages: [userMessage], currentIndex: 0 }]);
        scrollToBottom('smooth');
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col p-6 rounded-[20px]" onFocus={() => setIsChatVisible(true)}>
      {isInitializing ? (
      ...
      <div ref={chatContainerRef} className="flex-grow mb-6 overflow-y-auto scroll-smooth">
            ...
            <div key={index} className="flex flex-col items-start mb-4">
              <div className="max-w-[70%] p-3 rounded-lg bg-gray-200 text-gray-800">
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: formatMessage(item.messages[item.currentIndex]?.content || '') }}
                />
                {item.messages.length > 1 && (
                  <div className="mt-2 flex justify-between items-center">
                    <button 
                      onClick={() => {
                        setConversationItems(prev => prev.map((prevItem, i) => 
                          i === index ? {
                            ...prevItem,
                            currentIndex: (prevItem.currentIndex - 1 + prevItem.messages.length) % prevItem.messages.length
                          } : prevItem
                        ));
                      }}
                    ...
                    <button 
                      onClick={() => {
                        setConversationItems(prev => prev.map((prevItem, i) => 
                          i === index ? {
                            ...prevItem,
                            currentIndex: (prevItem.currentIndex + 1) % prevItem.messages.length
                          } : prevItem
                        ));
                      }}
                      className="text-sm text-gray-600 hover:text-gray-800"
            ...
            item.messages[0]?.content && (
              <div key={index} className="flex flex-col items-end mb-4">
                <div className="max-w-[70%] p-3 rounded-lg bg-blue-500 text-white">
                  ...
                  <div 
                    className="whitespace-pre-wrap font-sans"
                    dangerouslySetInnerHTML={{ __html: formatMessage(item.messages[0].content) }}
                  />
                </div>
              </div>
            )
      ...
      <div className=" flex justify-between px-3 items-center">
        ...
        style={{ transform: 'rotate(-45deg)' }}
          className={`ml-auto right-2 top-1/2 transform -translate-y-1/2 text-green-500 py-2 mb-2 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
